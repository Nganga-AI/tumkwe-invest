...
class TestInsightCard:
    def test_init(self):
        card = InsightCard("Revenue", "$1.5M", "Quarterly revenue", "+5%", "up")
...
class TestStockDashboard:
    ...
    def test_set_technical_indicators(self):
        dashboard = StockDashboard("AAPL", "Apple Inc.")
        indicators = {"rsi": [45, 55], "macd": [0.5, 0.6]}
    ...
    def test_set_sentiment_data(self):
        dashboard = StockDashboard("AAPL", "Apple Inc.")
        sentiment = {"overall_score": 0.75, "summary": "Positive sentiment"}
    ...
    def test_build_technical_section_basic_mode(self):
        ...
        rsi_cards = [
            c
            for c in components
            if c["type"] == "card" and "RSI" in c["config"]["title"]
        ]
        ...
        health_cards = [
            c
            for c in components
            if c["type"] == "card" and "Health" in c["config"]["title"]
        ]
    ...
    def test_build_fundamental_section(self):
        ...
        health_cards = [
            c
            for c in components
            if c["type"] == "card" and "Financial Health" in c["config"]["title"]
        ]
        assert len(health_cards) == 1
    ...
    def test_build_sentiment_section(self):
        ...
        dashboard.set_sentiment_data(
            {"overall_score": 0.75, "summary": "Positive sentiment"}
        )
        dashboard.build_sentiment_section()
    ...
    def test_build_dashboard(self):
        ...
        dashboard.set_fundamental_metrics(
            {"pe_ratio": {"value": 25, "industry_avg": 22}}
        )
        ...
        dashboard.set_fundamental_metrics(
            {
                ...
                "pe_ratio": {
                    "value": 20,
                    "industry_avg": 25,
                },  # Below industry for PE (positive)
            }
        )
        ...
        dashboard.set_fundamental_metrics(
            {
                ...
                "pe_ratio": {
                    "value": 30,
                    "industry_avg": 25,
                },  # Above industry for PE (negative)
            }
        )
