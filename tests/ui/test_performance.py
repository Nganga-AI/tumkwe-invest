...
import time
import json
from tumkwe_invest.ui.components import Card, Button, Chart
...
class TestLargeDataHandling:
    def test_chart_with_large_dataset(self):
        ...
        assert generation_time < 1.0, f"Chart generation took {generation_time:.2f} seconds"
        ...
        assert serialization_time < 0.5, f"JSON serialization took {serialization_time:.2f} seconds"
        assert len(json_data) < 10 * 1024 * 1024, "JSON output is too large"
    
    def test_dashboard_with_comprehensive_data(self, sample_dates, sample_prices, 
