...
class TestComponentCombinations:
        ...
        card.set_content(
            {
                ...
                "actions": [primary_btn.to_dict(), secondary_btn.to_dict()],
            }
        )
    ...
    def test_chart_in_card(self):
        ...
        chart_config.add_price_data(
            ["2023-01-01", "2023-01-02"], [100.0, 105.0], "Stock Price"
        )
    ...
    def test_insight_card_in_section(self):
        ...
        revenue_card = InsightCard("Revenue", "$1.5M", "Quarterly revenue", "+5%", "up")
        revenue_card.add_tooltip("Revenue increased by 5% compared to last quarter")

        profit_card = InsightCard("Profit", "$320K", "Quarterly profit", "-2%", "down")
...
class TestChartIntegrations:
    def test_sentiment_gauge_in_card(self):
        ...
        assert (
            abs(
                result["content"]["chart"]["chartConfig"]["data"]["datasets"][0][
                    "value"
                ]
                - 0.75
            )
        ...
        assert (
            result["content"]["interpretation"] == "Strongly positive market sentiment"
        )
