...
class TestPerformance(unittest.TestCase):
    ...
    def test_stock_data_collection_performance(self):
        ...
        for symbol in self.symbols:
            ...
            print(
                f"{symbol}: {len(prices) if prices else 0} records in {elapsed:.2f} seconds"
            )
    ...
    def test_news_collection_performance(self):
        ...
        for symbol in self.symbols:
            ...
            print(
                f"{symbol}: {len(articles) if articles else 0} articles in {elapsed:.2f} seconds"
            )
    ...
    def test_collector_manager_scalability(self):
        ...
        print(
            f"Found {len(due_tasks)} due tasks in {end_time - start_time:.6f} seconds"
        )

        # Performance assertion - adding a company should be fast
        self.assertLess(
            add_time / len(test_symbols), 0.5
        )  # Less than 0.5 seconds per company
