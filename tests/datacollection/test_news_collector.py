...
class TestNewsCollector(unittest.TestCase):
    ...
    def test_get_company_news(self, mock_get):
        ...
        self.assertEqual(
            results[0].summary, "Apple unveiled its new iPhone with improved features."
        )
        ...
        self.assertEqual(
            results[1].summary, "Apple Inc. reported record earnings for the quarter."
        )
    ...
    def test_get_company_news_with_content(self, mock_article_class, mock_get):
        ...
        self.assertEqual(
            results[0].content, "Full article content about the new iPhone."
        )
