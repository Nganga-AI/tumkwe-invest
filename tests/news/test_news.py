...
class TestNewsTools(unittest.TestCase):
    ...
    @patch(
        "yfinance.Search",
        side_effect=lambda query, enable_fuzzy_query, news_count: FakeSearch(
            query, enable_fuzzy_query, news_count
        ),
    )
    ...
    def test_fetch_company_news_fallback(self, mock_ticker, mock_search):
        ...
        expected_pub_time = datetime.fromtimestamp(1700000000).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        expected = [
            {
