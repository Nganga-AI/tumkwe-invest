...
class TestLLMProvider(unittest.TestCase):
    ...
    def test_get_anthropic_provider(self, mock_anthropic):
        ...
        provider = get_llm_provider(
            LLMProvider.ANTHROPIC, api_key="test-api-key", model="claude-3-opus"
        )

        # Verify ChatAnthropic was called with expected args
        mock_anthropic.assert_called_once_with(model="claude-3-opus", temperature=0.0)
    ...
    def test_get_groq_provider(self, mock_groq):
        ...
        # Get the provider
        provider = get_llm_provider("groq", api_key="test-api-key", temperature=0.7)

        # Verify ChatGroq was called with expected args
        mock_groq.assert_called_once_with(model="llama3-8b-8192", temperature=0.7)
    ...
    def test_llm_manager_change_provider(self, mock_get_provider):
        ...
        manager.change_provider(
            provider="anthropic", api_key="new-key", model="claude-3"
        )
        ...
        mock_get_provider.assert_called_with(
            provider="anthropic", api_key="new-key", model="claude-3", temperature=0.0
        )
