...
class AnalysisValidator:
    ...
    def validate_technical_indicators(
        ...
        if validation_results:
            ...
            total = sum(
                1 for result in validation_results.values() if isinstance(result, dict)
            )
    ...
    def validate_sentiment_model(
        test_dataset: List[Dict[str, str]], model_type: str = "ensemble"
    ) -> Dict[str, Any]:
        """
    ...
    def compare_recommendations(
        ...
        for source, recommendation in external_recommendations.items():
            ...
            agreement_level = (
                "high"
                if difference < 0.25
                else ("medium" if difference < 0.75 else "low")
            )
    ...
    def visualize_backtest(
        ...
        backtest_results = integrated.backtest(
            price_data, window_size=window_size, step_size=step_size
        )

        # Visualize if needed
        if (
            plot_file
            and hasattr(price_data, "index")
            and "detailed_results" in backtest_results
        ):
            ...
            correct = [
                r["correct_direction"] for r in backtest_results["detailed_results"]
            ]
            ...
            ax2.set_title(
                "Model Recommendation Scores (Green = Correct, Red = Incorrect)"
            )
