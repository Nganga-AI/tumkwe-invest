...
class Dashboard:
    ...
    def add_section(
        self, section_id: str, title: str, components: List[Dict[str, Any]]
    ) -> None:
        """
        ...
        """
        self.sections[section_id] = {"title": title, "components": components}
...
class StockDashboard(Dashboard):
    ...
    def set_price_data(
        self, dates: List[str], prices: List[float], volumes: Optional[List[int]] = None
    ) -> None:
        """Set historical price data for the dashboard."""
        self.price_data = {"dates": dates, "prices": prices, "volumes": volumes}
    ...
    def build_price_section(self) -> None:
        ...
        current_price = (
            self.price_data["prices"][-1] if self.price_data["prices"] else 0
        )
        prev_price = (
            self.price_data["prices"][-2]
            if len(self.price_data["prices"]) > 1
            else current_price
        )
    ...
    def build_technical_section(self) -> None:
        ...
        if self.view_mode == "basic":
            ...
            if "rsi" in self.technical_indicators:
                ...
                rsi_card = InsightCard(
                    ...
                    trend_direction=(
                        "down"
                        if current_rsi < 30
                        else "up" if current_rsi > 70 else "neutral"
                    ),
                )
                rsi_card.add_tooltip(
                    "Relative Strength Index measures momentum. RSI below 30 indicates oversold conditions, while above 70 indicates overbought conditions."
                )
            ...
            health_card.add_tooltip(
                "Technical Health is a simplified indicator combining multiple technical signals to give an overall assessment of price momentum."
            )
            ...
            if self.price_data.get("dates"):
                tech_chart = create_technical_indicators_chart(
                    self.price_data["dates"], self.technical_indicators
                )
    ...
    def build_fundamental_section(self) -> None:
        ...
        if self.view_mode == "basic":
            ...
            health_card.add_tooltip(
                "Financial Health combines multiple fundamental metrics to give an overall assessment of the company's financial position."
            )
            components.append({"type": "card", "config": health_card.to_dict()})
            ...
            if "pe_ratio" in self.fundamental_metrics:
                ...
                industry_pe = self.fundamental_metrics["pe_ratio"].get(
                    "industry_avg", 0
                )
                ...
                pe_status = (
                    ...
                    else "Higher than industry average"
                )
                ...
                pe_card.add_tooltip(
                    "Price-to-Earnings (P/E) ratio compares the company's share price to its earnings. Lower values may indicate the stock is undervalued, while higher values might suggest overvaluation."
                )
            ...
            if metrics and stock_values and industry_values:
                ...
                for name, data in self.fundamental_metrics.items():
                    ...
                    interpretation = self._get_fundamental_interpretation(
                        name, value, industry_avg
                    )
    ...
    def build_sentiment_section(self) -> None:
        ...
        sentiment_card = InsightCard(
            ...
            interpretation=self.sentiment_data.get(
                "summary", "Based on recent news and social media"
            ),
            trend_direction=(
                "up"
                if overall_score > 0
                else "down" if overall_score < 0 else "neutral"
            ),
        )
        sentiment_card.add_tooltip(
            "Market sentiment reflects the overall feeling or tone of investors toward a stock based on news, social media, and analyst opinions."
        )
    ...
    def build_integrated_section(self) -> None:
        ...
        if "integrated_score" in self.sentiment_data:
            ...
            integrated_card = InsightCard(
                ...
                trend_direction=(
                    "up" if score > 50 else "down" if score < 50 else "neutral"
                ),
            )
            integrated_card.add_tooltip(
                "Integrated Analysis Score combines technical, fundamental, and sentiment indicators to provide a comprehensive evaluation of the stock."
            )
    ...
    def _calculate_technical_health(self) -> Dict[str, str]:
        ...
        # Check MACD
        if (
            "macd" in self.technical_indicators
            and "macd_signal" in self.technical_indicators
        ):
            if (
                len(self.technical_indicators["macd"]) > 1
                and len(self.technical_indicators["macd_signal"]) > 1
            ):
        ...
        if "moving_averages" in self.technical_indicators:
            for ma_name, ma_values in self.technical_indicators[
                "moving_averages"
            ].items():
    ...
    def _get_fundamental_interpretation(
        self, metric: str, value: float, industry_avg: Optional[float] = None
    ) -> str:
        ...
        percent_diff = (
            ((value - industry_avg) / industry_avg * 100) if industry_avg != 0 else 0
        )
    ...
    def _get_fundamental_tooltip(self, metric: str) -> str:
        ...
        return tooltips.get(
            metric, "Financial metric used to evaluate company performance"
        )
    ...
    dashboard.set_technical_indicators(
        {
            ...
            "moving_averages": {"MA50": ma50, "MA200": ma200},
        }
    )
