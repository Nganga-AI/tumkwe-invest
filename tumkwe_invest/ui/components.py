...
class Tooltip(UIComponent):
    ...
    def __init__(
        self, content: str, position: str = "top", component_id: Optional[str] = None
    ):
        """
...
class ToggleSwitch(UIComponent):
    ...
    def __init__(
        self, label: str, checked: bool = False, component_id: Optional[str] = None
    ):
        """
...
def create_tooltip_with_info(target_id: str, content: str) -> Dict[str, Any]:
    ...
    """
    tooltip = Tooltip(
        content=content, position="top", component_id=f"{target_id}-tooltip"
    )
...
def create_metric_card(
    ...
    card = Card(title=title, component_id=card_id)
