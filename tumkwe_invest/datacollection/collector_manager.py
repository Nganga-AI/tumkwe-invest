...
from .config import (
    DATA_REFRESH_INTERVAL, CACHE_DIRECTORY, CACHE_EXPIRY
)
...
from .collectors.sec_edgar import get_recent_filings, download_filing_document
from .validation import (
    validate_stock_prices, validate_financial_statement, validate_key_metrics,
    validate_company_profile, validate_news_articles, generate_combined_report
)
...
class CollectorManager:
    ...
    def remove_company(self, symbol: str):
        ...
        self.tasks = [
            ...
            if symbol not in task.company_symbols
        ]
            ...
            try:
                ...
                if prices:
                    ...
                    if os.path.exists(prices_file):
                        ...
                        combined_prices = combined_prices.drop_duplicates(subset=['date'])
            ...
            try:
                ...
                if profile:
                    ...
                    with open(profile_file, 'w') as f:
                        ...
                        if 'ipo_date' in profile_dict and profile_dict['ipo_date']:
                            profile_dict['ipo_date'] = profile_dict['ipo_date'].isoformat()
                ...
                for statement_type, statement_list in statements.items():
                    if statement_list:
                        ...
                        # Save statement data
                        statement_file = os.path.join(company_dir, f"{statement_type}.json")
                        with open(statement_file, 'w') as f:
                            statements_dict = []
                            for statement in statement_list:
                                stmt_dict = {k: v for k, v in statement.__dict__.items() 
                                          if not k.startswith('_') and k != 'validation_warnings'}
                                # Handle datetime
                                stmt_dict['date'] = stmt_dict['date'].isoformat()
                                stmt_dict['last_updated'] = stmt_dict['last_updated'].isoformat()
                ...
                for statement_type, statement_list in qtr_statements.items():
                    if statement_list:
                        ...
                        statement_file = os.path.join(company_dir, f"quarterly_{statement_type}.json")
                        with open(statement_file, 'w') as f:
                            statements_dict = []
                            for statement in statement_list:
                                stmt_dict = {k: v for k, v in statement.__dict__.items() 
        ...
        for symbol in task.company_symbols:
            try:
                ...
                if news_api_articles:
                    # Validate the news
                    validation_report = validate_news_articles(news_api_articles, symbol)
                ...
                if all_news:
                    ...
                    news_df = pd.DataFrame([
                        {
                            ...
                            'is_valid': article.relevance_score is not None  # Use relevance as validation
                        }
                    ...
                    for i, article in enumerate(all_news):
                        if article.content:
                            article_filename = f"article_{article.date.strftime('%Y%m%d')}_{i}.txt"
                ...
                logger.info(f"News collection complete for {symbol}: {len(all_news)} articles")
                    ...
                    for filing in filings:
                        if filing.filing_type in important_filings:
                            ...
                            if content:
                                ...
                                with open(os.path.join(filings_dir, filing_filename), "w", encoding="utf-8") as f:
                                    f.write(content)
                
                logger.info(f"SEC filings collection complete for {symbol}: {len(filings)} filings")
