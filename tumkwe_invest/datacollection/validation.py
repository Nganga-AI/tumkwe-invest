...
def validate_stock_prices(prices: List[StockPrice], symbol: str) -> ValidationReport:
    ...
    for i, price in enumerate(prices):
        ...
        if i > 0:
            ...
            daily_change_pct = abs((price.close - prev_price.close) / prev_price.close * 100)
            if daily_change_pct > VALIDATION["max_price_change_percent"]:
                issues.append(f"Extreme daily price change: {daily_change_pct:.1f}%")
        
        # Check for outliers
        if abs(price.close - mean_price) > VALIDATION["max_outlier_std"] * std_dev and std_dev > 0:
            issues.append(f"Price is an outlier: {price.close:.2f} vs mean {mean_price:.2f}")
...
def validate_financial_statement(statement: FinancialStatement) -> ValidationReport:
    ...
    if missing_fields:
        report.issues["missing_fields"] = [f"Missing key fields: {', '.join(missing_fields)}"]
    ...
    if ("Net Income" in statement.data and "Total Revenue" in statement.data and 
            statement.data["Total Revenue"] > 0):
    ...
    if statement.statement_type == "balance_sheet":
        if ("Total Assets" in statement.data and 
                "Total Liabilities" in statement.data and 
                "Total Equity" in statement.data):
            
            assets = statement.data["Total Assets"]
            liab_equity = statement.data["Total Liabilities"] + statement.data["Total Equity"]
            
            # Allow small rounding differences (0.5%)
            if abs(assets - liab_equity) / assets > 0.005:
                issues.append(f"Balance sheet equation doesn't balance: Assets={assets:.2f}, Liabilities+Equity={liab_equity:.2f}")
...
def validate_key_metrics(metrics: KeyMetrics) -> ValidationReport:
    ...
    if metrics.dividend_yield is not None and metrics.dividend_yield > 0.25:  # 25% yield is very high
        issues.append(f"Unusually high dividend yield: {metrics.dividend_yield*100:.2f}%")
...
def validate_news_articles(articles: List[NewsArticle], symbol: str) -> ValidationReport:
    """
    ...
    for i, article in enumerate(articles):
        ...
        if article.title and symbol.lower() not in article.title.lower() and symbol.lower() not in article.summary.lower():
            relevance_issue = "Article may not be directly relevant to the company"
            if not any(issues):  # Don't add this issue if there are already more serious ones
...
def generate_combined_report(reports: List[ValidationReport]) -> Dict[str, Any]:
    ...
    if combined["total_records"] > 0:
        combined["validation_rate"] = combined["valid_records"] / combined["total_records"]
    ...
    for report in reports:
        ...
        for issue_key, issue_list in report.issues.items():
            ...
            for issue in issue_list:
                combined["issues_by_type"][issue_key].append(f"{report.data_type} ({report.company_symbol}): {issue}")
