...
from ..config import SEC_USER_AGENT, API_RATE_LIMITS, CACHE_DIRECTORY
...
def get_recent_filings(ticker: str, filing_types: List[str] = None, count: int = 10) -> List[SECFiling]:
    """
                ...
                try:
                    ...
                    if filing_type in filing_types:
                        ...
                        if doc_link and 'href' in doc_link.attrs:
                            ...
                            accession_match = re.search(r'/(\d{10}-\d{2}-\d+)/', doc_url)
...
def download_filing_document(filing: SECFiling) -> Optional[str]:
    ...
    cache_file = os.path.join(CACHE_DIRECTORY, "sec_filings", 
    ...
    try:
        ...
        print(f"Could not find document content for {filing.company_symbol} {filing.filing_type}")
        return None
