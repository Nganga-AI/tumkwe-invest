...
class IntegratedAnalyzer:
    ...
    def get_technical_score(self) -> Dict[str, Any]:
        ...
        if not self.technical_analyzer:
            return {"score": 0, "confidence": 0, "details": "No technical analyzer provided"}
        ...
        strength = min(max(trend["strength"], -3), 3) / 3  # Clamp to -3 to 3, then normalize
        ...
        confidence = min(indicator_count / 5, 1.0)  # At least 5 indicators for max confidence
    ...
    def get_fundamental_score(self) -> Dict[str, Any]:
        ...
        if not self.fundamental_analyzer:
            return {"score": 0, "confidence": 0, "details": "No fundamental analyzer provided"}
    ...
    def get_sentiment_score(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict: Score details with value between -1 and 1
        """
        if not self.sentiment_analyzer or not hasattr(self.sentiment_analyzer, "results") or not self.sentiment_analyzer.results:
            return {"score": 0, "confidence": 0, "details": "No sentiment results available"}
        
        results = self.sentiment_analyzer.results
        
        if "overall" not in results:
            return {"score": 0, "confidence": 0, "details": "No overall sentiment found"}
    ...
    def calculate_integrated_score(self) -> Dict[str, Any]:
        ...
        technical_contrib = technical_result["score"] * self.weights["technical"] * technical_result["confidence"]
        fundamental_contrib = fundamental_result["score"] * self.weights["fundamental"] * fundamental_result["confidence"]
        sentiment_contrib = sentiment_result["score"] * self.weights["sentiment"] * sentiment_result["confidence"]
        ...
        if total_weighted_confidence > 0:
            final_score = (technical_contrib + fundamental_contrib + sentiment_contrib) / total_weighted_confidence
        else:
            final_score = 0
        ...
        conflicts = self.detect_conflicts(technical_result, fundamental_result, sentiment_result)
    ...
    def detect_conflicts(
        ...
        if technical_result["score"] * fundamental_result["score"] < 0:
            if abs(technical_result["score"]) > 0.3 and abs(fundamental_result["score"]) > 0.3:
                conflicts.append({
                    ...
                    "severity": "high" if abs(technical_result["score"] - fundamental_result["score"]) > 1.0 else "medium"
                })
        
        # Check for technical vs sentiment conflicts
        if technical_result["score"] * sentiment_result["score"] < 0:
            if abs(technical_result["score"]) > 0.3 and abs(sentiment_result["score"]) > 0.3:
                conflicts.append({
        ...
        if fundamental_result["score"] * sentiment_result["score"] < 0:
            if abs(fundamental_result["score"]) > 0.3 and abs(sentiment_result["score"]) > 0.3:
                conflicts.append({
    ...
    def backtest(
        ...
        for start_idx in range(0, len(historical_data) - window_size, step_size):
            ...
            if end_idx < len(historical_data) - 1:  # Ensure we have future data to compare
                future_return = (historical_data["close"].iloc[end_idx + 1] - 
                                window_data["close"].iloc[-1]) / window_data["close"].iloc[-1]
            else:
                future_return = None
            ...
            if future_return is not None:
                ...
                predicted_direction = np.sign(result["score"]) if result["score"] != 0 else 0
                ...
                backtest_results.append({
                    "date": window_data.index[-1] if hasattr(window_data, "index") else end_idx,
                    "score": result["score"],
        ...
        for result in backtest_results:
            ...
            position_return = position_size * position_direction * result["future_return"]
